...
public class Boar extends Animal implements Herbivores, Logging, HerbivoresFindPrey {
    public Boar(String name, double weight, int quantity, int foodNeeded) {
    ...
    public void move(Cell[][] cells, int x, int y) {
        ...
        if (emptyCell != null) {
            ...
            System.out.printf("Кабан переместился с (%d, %d) на (%d, %d)\n", x, y, emptyCell.getX(), emptyCell.getY());
        }
    }
    ...
    public void reproduce(Field field) {
        if (this.getSaturation() >= this.getFoodNeeded()) {
            ...
            if (randomEmptyCell != null) {
                ua.com.javarush.gnew.herbivore.Boar newBoar = new ua.com.javarush.gnew.herbivore.Boar("Кабан", 400, 1, 50);
                randomEmptyCell.setAnimal(newBoar);

                System.out.printf("Кабан размножился в клетке (%d, %d)\n", randomEmptyCell.getX(), randomEmptyCell.getY());
