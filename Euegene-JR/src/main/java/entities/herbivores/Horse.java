...
public class Horse extends Herbivore{
    ...
    public Horse(Location currentLocation) {
    ...
    public void reproduce() {
        Set<Animal> sameResidents = currentLocation.getResidents().get(Horse.class);
        if (sameResidents.size()>1){
            if(random.nextInt(100)<REPRODUCE_PROBABILITY*100){
                if (health > HEALTH_TO_REPRODUCE) {
                    decreaseHealth(10);
                    currentLocation.addAnimal(new Horse(currentLocation));
                }
                checkOnDeath();
            }
        }
