...
public class Mouse extends Herbivore{
    ...
    public Mouse(Location currentLocation) {
    ...
    public void reproduce() {
        Set<Animal> sameResidents = currentLocation.getResidents().get(Mouse.class);
        if (sameResidents.size()>1){
            if(random.nextInt(100)<REPRODUCE_PROBABILITY*100){
                if (health > HEALTH_TO_REPRODUCE) {
                    decreaseHealth(10);
                    currentLocation.addAnimal(new Mouse(currentLocation));
                }
                checkOnDeath();
            }
        }
